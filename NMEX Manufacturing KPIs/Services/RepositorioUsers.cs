...
namespace NMEX_Manufacturing_KPIs.Services
{
    ...
    public class RepositorioUsers : IRepositorioUsers
    {
        ...
        public async Task<IEnumerable<Users>> GetUsersRecords()
        {
            ...
            return await connection.QueryAsync<Users>(@"SELECT U.FirstName, U.LastNameMaternal, U.LastNamePaternal, U.Email, U.Password, U.Privilege, U.Active, P.Plant_description as Plant
        ...
        public async Task<int> CreateUser(Users user)
        {
            ...
            using var connection = new SqlConnection(connectionString);
            var user_id = await connection.QuerySingleAsync<int>(@"INSERT INTO Users (FirstName, LastNamePaternal, LastNameMaternal, Email, EmailNormalized, Password, Privilege, Active, Plant_id)
                                                                    VALUES (@FirstName, @LastNamePaternal, @LastNameMaternal, @Email, @EmailNormalized, @Password,@Privilege, @Active, @Plant_id)
